...
class resultClass:
    def __init__(self, root):
        ...
        # for title
        title = Label(self.root, text="Add student result", font=("goudy old style", 20,
                      "bold"), bg="#033054", fg="white").place(x=10, y=15, width=1180, height="50")
        # widgets        # Variables
        ...
        btn_search = Button(self.root, text='Search', font=("goudy old style", 15, "bold"), bg="#03a9f4",
                            fg="white", cursor="hand2", command=self.search).place(x=500, y=100, width=100, height=28)
        txt_name = Entry(self.root, textvariable=self.var_name, font=(
            "goudy old style", 20, 'bold'), bg='#e6f3ff', state='readonly').place(x=280, y=160, width=320)
        txt_course = Entry(self.root, textvariable=self.var_course, font=(
            "goudy old style", 20, 'bold'), bg='#e6f3ff', state='readonly').place(x=280, y=220, width=320)
        txt_marks = Entry(self.root, textvariable=self.var_marks, font=(
            "goudy old style", 20, 'bold'), bg='#e6f3ff').place(x=280, y=280, width=320)
        txt_full_marks = Entry(self.root, textvariable=self.var_full_marks, font=(
            "goudy old style", 20, 'bold'), bg='#e6f3ff').place(x=280, y=340, width=320)
        # button
    ...
    def add(self):
        ...
        try:
            if self.var_name.get() == "":
                ...
                if row != None:
                    messagebox.showerror(
                        "Error", "Result is already present", parent=self.root)
                else:
